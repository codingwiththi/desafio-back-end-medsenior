...
export class AiService {
  ...
  private isValidApiKey(apiKey: string): boolean {
    return Boolean(apiKey && apiKey.length > 0 && apiKey.startsWith('sk-') && !apiKey.includes('your-openai-api-key'));
  }
  ...
  async askQuestion(question: string): Promise<AiResponse> {
    try {
      ...
      const response = await axios.post(
        `${this.baseUrl}/chat/completions`,
        {
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a helpful assistant. Provide clear, accurate, and concise answers.',
            },
            {
              role: 'user',
              content: question,
      ...
      const answer = response.data.choices[0]?.message?.content || 'I apologize, but I could not generate a response at this time.';
...
*Note: This is a mock response for development purposes.*`;
    } else if (questionLower.includes('machine learning') || questionLower.includes('ml')) {
...
*Note: This is a mock response for development purposes.*`;
    } else if (questionLower.includes('programming') || questionLower.includes('code')) {
...
*Note: This is a mock response for development purposes.*`;
      ...
      response = genericResponses[Math.floor(Math.random() * genericResponses.length)] || 'Thank you for your question. This is a mock response for development purposes.';
    }

    logger.info('Mock AI response generated', {
      questionLength: question.length,
      answerLength: response.length,
      questionType: questionLower.includes('ai') || questionLower.includes('artificial intelligence') ? 'AI-related' : 'general'
    });
